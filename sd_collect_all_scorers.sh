...
if [ -z "${SLURM_JOB_ID:-}" ]; then
    echo "This script is meant to be run via sbatch (usage: sbatch sd_collect_all_scorers.sh)." >&2
    exit 1
fi
...
for scorer in clip brightness compressibility; do
  python main.py \
    --backend sd \
    --scorer "$scorer" \
    --method eps_greedy \
    ...
    --search_log_prefix "sd_${scorer}_epsgreedy_first20"
done
