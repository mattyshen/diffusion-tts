...
if [ -z "${SLURM_JOB_ID:-}" ]; then
  echo "Run with: sbatch scripts/inefficiency/SD/brightness.sh" >&2
  exit 1
fi
...
MODEL_NAME="SD"
REWARD_NAME="BRIGHTNESS"

PROMPT_SRC="prompts_imagenet_first20.txt"
RUN_TAG="${METHOD}_p${N_PROMPTS}_seed${SEED}_N${N}_K${K}_eps${EPS}_lam${LAMBDA}"
...
echo "[run] prompt file: ${PROMPT_FILE}"
